...
for target in ${!merge_dict[@]}; do
    ...
    echo "srun -c $p samtools merge -@ $p - $in_files | srun -c $p samtools sort -@ $p - -o $out_file &" >> $log
    srun -c $p samtools merge -@ $p - $in_files | srun -c $p samtools sort -@ $p - -o $out_file &
done
wait
...
for target in ${!merge_dict[@]}; do
    ...
    srun -c $p samtools index -@ $p $in_file &
done
wait
...
for target in ${!merge_dict[@]}; do
    ...
    echo "srun -c $p bamCoverage -b $in_file -o $out_file -p $p --binSize 10 --smoothLength 50 --normalizeUsing CPM &" >> $log
    srun -c $p bamCoverage -b $in_file -o $out_file -p $p --binSize 10 --smoothLength 50 --normalizeUsing CPM &
done
